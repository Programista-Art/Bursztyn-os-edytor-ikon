...
/* =========================================================================
 * 5.1 RYSOWANIE IKON Z DANYCH - .data zamiast setek wywolan w .text
 * ========================================================================= */

void rysuj_ikone_paleta(int x,
                        int y,
                        const uint32_t* paleta,
                        uint32_t liczba_kolorow,
                        const FragmentIkonyPulpitu* fragmenty,
                        uint32_t liczba_fragmentow) {
    if (!paleta || !fragmenty) return;
    if (liczba_kolorow == 0) return;
    for (uint16_t i = 0; i < liczba_fragmentow; ++i) {
        const FragmentIkonyPulpitu& f = fragmenty[i];
        /* Walidacja: idx < liczba_kolorow, wymiary > 0, brak przeciecia uint8. */
        if (f.kolor >= liczba_kolorow) continue;
        if (f.szer == 0 || f.wys == 0) continue;
        gui_rysuj_prostokat(
            x + static_cast<int>(f.x),
            y + static_cast<int>(f.y),
            static_cast<int>(f.szer),
            static_cast<int>(f.wys),
            paleta[f.kolor]);
    }
}
